...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        if return_attention:
            # return x, self.diversity_loss_rn573(attn)
            return x, attn
        return x
...
class PiT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 isVideo=False, spatial=True, temporal=False, vis=False):
        super().__init__()
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        # 1. Patch Embedding
        B, N, C, H, W = x.shape
        x = self.patch_embed(x.contiguous().view(-1, C, H, W))
        _, M, D = x.shape
        camera_id = [i for i in camera_id for j in range(N)]
        cls_tokens = self.cls_token

        # 2. Spatial Transformer
        if self.spatial:
            # 2.1. add cls_token and pos_embedding
            cls_tokens = cls_tokens.expand(B*N, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            ...
            for blk in self.spatial_blocks:
                if self.vis:
                    ...
                    x = blk(x)
            # 2.3. average image features
            # x_agg = self.aggregator(x.view(B, N, -1 ,self.num_features))
            # return x_agg
            if self.vis:
                return x, attns
            else:
    ...
    def load_spatiotemporal_param(self, model_path):
        if self.spatial:
            ...
            for k, v in param_dict.items():
                ...
                try:
                    if 'blocks.11' in k:
                        k = k.replace('blocks.11', 'head_block')
                    elif 'blocks' in k:
                        k = 'spatial_' + k
...
def vit_small_patch16_224_PiT(img_size=(256, 128), stride_size=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
...
def deit_small_patch16_224_PiT(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, drop_rate=0.0, attn_drop_rate=0.0, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = PiT(
